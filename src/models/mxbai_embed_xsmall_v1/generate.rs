...
use crate::models::{
    ...
    mxbai_embed_xsmall_v1::model::MxbaiEmbedXsmallV1Backend,
};

pub struct MxbaiEmbedXsmallV1Model {
    backend: MxbaiEmbedXsmallV1Backend,
}

impl MxbaiEmbedXsmallV1Model {
        ...
        match spec.resolved_artifact() {
            ArtifactKind::Safetensors => {
                let path = spec.paths.weight_dir.as_deref().ok_or_else(|| {
                    anyhow::anyhow!("weight_path is required for mxbai-embed-xsmall-v1 safetensors")
                })?;
                Self::init(path, device, dtype)
            }
            ArtifactKind::Gguf => {
                let path = spec.paths.gguf_path.as_deref().ok_or_else(|| {
                    anyhow::anyhow!("gguf_path is required for mxbai-embed-xsmall-v1 gguf")
                })?;
                Self::init_gguf(path, spec.paths.tokenizer_dir.as_deref(), device, dtype)
            }
            ArtifactKind::Onnx => {
                let path = spec.paths.onnx_path.as_deref().ok_or_else(|| {
                    anyhow::anyhow!("onnx_path is required for mxbai-embed-xsmall-v1 onnx")
                })?;
    ...
    pub fn init(path: &str, device: Option<&Device>, dtype: Option<DType>) -> Result<Self> {
        let backend = MxbaiEmbedXsmallV1Backend::load(path, device, dtype)?;
        Ok(Self { backend })
    }

    pub fn init_onnx(onnx_path: &str, tokenizer_dir: Option<&str>) -> Result<Self> {
        let backend = MxbaiEmbedXsmallV1Backend::load_onnx(onnx_path, tokenizer_dir)?;
        Ok(Self { backend })
    }
    ...
    pub fn init_gguf(
        ...
        let backend =
            MxbaiEmbedXsmallV1Backend::load_gguf(gguf_path, tokenizer_dir, device, dtype)?;
        Ok(Self { backend })
    }
...
impl TextEmbeddingBackend for MxbaiEmbedXsmallV1Model {
