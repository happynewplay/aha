...
impl EmbeddingPromptName {
    pub fn parse_api_str(value: &str) -> Result<Self> {
        match value {
            ...
            _ => Err(anyhow!("unsupported embedding prompt_name: {}", value)),
        }
    }
}
...
pub fn load_model_legacy<'a>(
    ...
    let model = match model_type {
        WhichModel::MiniCPM5_1B => {
            let model = if let Some(gguf) = gguf {
                MiniCPM5GenerateModel::init_from_gguf(gguf, None, None)?
            } else {
                MiniCPM5GenerateModel::init(path, None, None)?
            };
